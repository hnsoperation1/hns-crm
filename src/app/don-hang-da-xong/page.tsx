...
export default function DaXongPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data }, { data: userData }] = await Promise.all([
      supabase
        .from('opportunities')
        .select('id, title, description, source, tour_date, tour_end_date, estimated_value, actual_value, contact:contacts(name, company), assigned_user:users!assigned_to(id, full_name), creator:users!created_by(id, full_name)')
        .is('deleted_at', null)
        .eq('stage', 'stage_5')
  ...
  const totalActual = filtered.reduce((s, r) => s + (r.actual_value ?? 0), 0)
  const totalEst = filtered.reduce((s, r) => s + (r.estimated_value ?? 0), 0)

  const cols = ['Đơn hàng', 'Nguồn', 'Sale TV', 'Điểm đến', 'Ngày đi', 'Ngày về', 'Doanh thu TT', 'Giá ước tính', 'Chênh lệch']
            ...
            {loading ? (
              Array.from({ length: 8 }).map((_, i) => (
                <tr key={i} className="animate-pulse">
                  {[38, 14, 18, 18, 12, 12, 14, 14, 12].map((w, j) => (
              ...
              <>
                ...
                <tr className="bg-gray-50 border-t-2 border-gray-200">
                  <td colSpan={6} className="px-5 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">{filtered.length} đơn</td>
        ...
        <div className="fixed top-0 right-0 h-full w-[480px] bg-white shadow-2xl z-50 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            {[
              ...
              { label: 'Doanh thu TT', value: slideRow.actual_value ? formatVND(slideRow.actual_value) : '—' },
              { label: 'Giá ước tính', value: slideRow.estimated_value ? formatVND(slideRow.estimated_value) : '—' },
